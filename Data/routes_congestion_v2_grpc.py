...
#   3. Set up Application Default Credentials for gRPC:
#        export GOOGLE_APPLICATION_CREDENTIALS="/path/to/service-account-key.json"
#      or authenticate with gcloud CLI.
#   4. Run: python routes_congestion_v2_grpc.py
#   5. Optionally, pass origin/destination lat/lng as arguments:
#      python routes_congestion_v2_grpc.py <origin_lat> <origin_lng> <dest_lat> <dest_lng>
#
# Example coordinates:
...
def main():
    ...
    metadata = [
        ...
        ("x-goog-fieldmask", "routes.duration,routes.staticDuration,routes.distanceMeters,routes.routeLabels,routes.legs.startLocation,routes.legs.endLocation"),
    ]
    ...
    distance_km = route.distance_meters / 1000 if hasattr(route, "distance_meters") else None
    ...
    if duration_seconds:
        print(f"Duration (with traffic): {duration_seconds} seconds ({duration_minutes:.2f} minutes)")
    ...
    try:
        response_unaware = client.compute_routes(request=request_unaware, metadata=metadata)
        if response_unaware.routes:
            ...
            duration_unaware_minutes = duration_unaware_seconds / 60 if duration_unaware_seconds else None
            if duration_unaware_seconds:
                print(f"Duration (no traffic): {duration_unaware_seconds} seconds ({duration_unaware_minutes:.2f} minutes)")
            if duration_seconds and duration_unaware_seconds:
                diff_aware_unaware = duration_seconds - duration_unaware_seconds
                percent_aware_unaware = (diff_aware_unaware / duration_unaware_seconds) * 100
                print(f"Real-time vs No traffic: +{percent_aware_unaware:.1f}% travel time (current vs ideal)")
                if percent_aware_unaware < 10:
                    congestion_status = "SMOOTH (estimated)"
                elif percent_aware_unaware < 30:
                    congestion_status = "MODERATE (estimated)"
                elif percent_aware_unaware < 60:
                    congestion_status = "SLOW (estimated)"
                else:
                    congestion_status = "SEVERE (estimated)"
                print(f"Traffic condition (estimated): {congestion_status}")
        else:
            print("Could not estimate traffic condition (no route for traffic-unaware).")
    except Exception as e:
